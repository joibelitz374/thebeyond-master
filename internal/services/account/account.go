...
type Interface interface {
	...
	Create(ctx context.Context, platform consts.Platform, platformAccountID int, promo *string, discount int) (int, string, error)
...
func (s service) Create(ctx context.Context, platform consts.Platform, platformAccountID int, promo *string, discount int) (int, string, error) {
	keyID, err := gonanoid.Generate(consts.NANOID_ALPHABET, consts.NANOID_LENGTH)
	if err != nil {
		return 0, "", err
	}

	newAccountID, err := s.repo.Create(ctx, platform, platformAccountID, keyID, values.RandomShortId(), promo, discount)
	return newAccountID, keyID, err
}
