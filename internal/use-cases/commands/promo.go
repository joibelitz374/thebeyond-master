...
func (h promoHandler) statisticPromo(promo domain.Promo, msg i18n.PromoInfoLocale) string {
	text := ""
	...
	text += fmt.Sprintf("\n\n%s:", msg.PromocodeBuyers)
	for _, count := range promoRewards.BuyerTargets {
		text += formatAchievement(count, promo.Buyers, promoRewards.BuyerRewards[count], msg)
	}
